...
from ktlkuda.exceptions import MethodNotAllowed405Error
...
getLogger("ktlkuda").addHandler(NullHandler())
...
class Request(object):
    ...
    def _initialize_lesser(
        ...
        if bool(DISABLE_LOGGING_DEBUG_OUTPUT):
            for logger_ in ("ktlkuda", "requests", "urllib3"):
        ...
        if not self.host:
            env_host = environ.get(
                "ktlkuda_HOST", environ.get("PYTHON_ktlkuda_HOST")
            )
            if env_host is not None:
                logger.debug(
                    "Using ktlkuda_HOST env variable for qBittorrent host"
                )
                self.host = env_host
        if not self.username:
            env_username = environ.get(
                "ktlkuda_USERNAME", environ.get("PYTHON_ktlkuda_USERNAME")
            )
            if env_username is not None:
                logger.debug("Using ktlkuda_USERNAME env variable for username")
                self.username = env_username
        if not self._password:
            env_password = environ.get(
                "ktlkuda_PASSWORD", environ.get("PYTHON_ktlkuda_PASSWORD")
            )
            if env_password is not None:
                logger.debug("Using ktlkuda_PASSWORD env variable for password")
                self._password = env_password
        if self._VERIFY_WEBUI_CERTIFICATE is True:
            env_verify_cert = environ.get(
                "ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE",
                environ.get("PYTHON_ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE"),
            )
    ...
    def _get(
        ...
        **kwargs
        ...
        :param api_namespace: the namespace for the API endpoint
            (e.g. :class:`~ktlkuda.definitions.APINames` or ``torrents``)
        ...
        **kwargs
        ...
        def build_error_msg(exc):
            ...
            error_messages = {
                ...
                "environment variable PYTHON_ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE "
