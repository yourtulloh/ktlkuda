...
from ktlkuda import APINames
from ktlkuda import Client
from ktlkuda import exceptions
from ktlkuda._version_support import v
...
def test_log_in_via_auth():
    ...
    assert (
        client_good.auth_log_in(
            username=environ.get("ktlkuda_USERNAME"),
            password=environ.get("ktlkuda_PASSWORD"),
        )
        is None
    )
    with pytest.raises(exceptions.LoginFailed):
...
def test_port_from_host(app_version):
    host, port = environ.get("ktlkuda_HOST").split(":")
...
def test_force_user_scheme(client, app_version, use_https):
    default_host = environ["ktlkuda_HOST"]
...
def test_legacy_env_vars():
    save_env_host = environ.get("ktlkuda_HOST")
    save_env_username = environ.get("ktlkuda_USERNAME")
    save_env_password = environ.get("ktlkuda_PASSWORD")
    save_env_verify = environ.get("ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE")
    del environ["ktlkuda_HOST"]
    del environ["ktlkuda_USERNAME"]
    del environ["ktlkuda_PASSWORD"]
    if "ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE" in environ:
        del environ["ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE"]
    ...
    environ["PYTHON_ktlkuda_HOST"] = excepted_host = "legacy:8090"
    environ["PYTHON_ktlkuda_USERNAME"] = excepted_username = "legacyuser"
    environ["PYTHON_ktlkuda_PASSWORD"] = expected_password = "legacypassword"
    environ["PYTHON_ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE"] = "true"
    ...
    try:
        ...
        environ["ktlkuda_HOST"] = save_env_host
        environ["ktlkuda_USERNAME"] = save_env_username
        environ["ktlkuda_PASSWORD"] = save_env_password
        if save_env_verify is not None:
            environ["ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE"] = save_env_verify
    ...
    del environ["PYTHON_ktlkuda_HOST"]
    del environ["PYTHON_ktlkuda_USERNAME"]
    del environ["PYTHON_ktlkuda_PASSWORD"]
    del environ["PYTHON_ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE"]
...
def test_disable_logging():
    ...
    assert logging.getLogger("ktlkuda").level == logging.NOTSET
    ...
    assert logging.getLogger("ktlkuda").level == logging.INFO
    assert logging.getLogger("requests").level == logging.INFO
    assert logging.getLogger("urllib3").level == logging.INFO

    logging.getLogger("ktlkuda").setLevel(level=logging.CRITICAL)
    Client(DISABLE_LOGGING_DEBUG_OUTPUT=True)
    assert logging.getLogger("ktlkuda").level == logging.CRITICAL
    assert logging.getLogger("requests").level == logging.INFO
    assert logging.getLogger("urllib3").level == logging.INFO
...
def test_verify_cert(app_version):
    ...
    environ["ktlkuda_DO_NOT_VERIFY_WEBUI_CERTIFICATE"] = "true"
...
def test_request_retry_success(monkeypatch, caplog):
    ...
    with monkeypatch.context() as m:
        ...
        with caplog.at_level(logging.DEBUG, logger="ktlkuda"):
